...
def valid(args, model, writer, test_loader, global_step):
    ...
    epoch_iterator = tqdm(test_loader,
                          desc="Validating... (cls_loss=X.X) (shape_loss=X.X)",
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        x, y = batch
        with torch.no_grad():
            logits,assign, attn_weights = model(x)
            cls_loss = loss_fct(logits, y)
            shaping_loss = ShapingLoss(assign, 2, 0.4, args.num_parts, 1, 0.001)
            loss = cls_loss + args.shape_loss_weight*shaping_loss
            cls_losses.update(cls_loss.item())
            shaping_losses.update(shaping_loss.item())
        ...
        epoch_iterator.set_description("Validating... (cls_loss=%2.5f) (shape_loss=%2.5f)" %(cls_losses.val,shaping_losses.val))
...
def train(args, model):
    ...
    optimizer = torch.optim.SGD([{'params':model.inter_arch.parameters(),'lr':args.learning_rate},
                                 {'params':model.grouping.parameters(),'lr':5 * args.learning_rate},
                                 {'params':model.vit.transformer.parameters(),'lr':10 * args.learning_rate},
                                 {'params':model.vit.head.parameters(),'lr':20 * args.learning_rate},
                                 {'params': model.vit.mask_head.parameters(), 'lr': 20 * args.learning_rate},
                                 {'params': model.vit.mask_cls_head.parameters(), 'lr': 20 * args.learning_rate}],
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            batch = tuple(t.to(args.device) for t in batch)
            x, y = batch
            cls_loss, mask_ce_loss, bce_loss, logits, assign = model(x, y)
            #loss = loss.mean() # for contrastive learning
            shaping_loss = ShapingLoss(assign, 2, 0.4, args.num_parts, 1, 0.001)
            loss = cls_loss + args.shape_loss_weight*shaping_loss
            if mask_ce_loss is not None:
                loss = loss + args.mask_cls_weight * mask_ce_loss
                # loss = loss + 0 * mask_ce_loss
            else:
                mask_ce_loss = torch.Tensor([0])
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                shaping_losses.update(shaping_loss.item()*args.gradient_accumulation_steps)
                losses.update(loss.item()*args.gradient_accumulation_steps)
                bce_losses.update(bce_loss.item() * args.gradient_accumulation_steps)
                mask_ce_losses.update(mask_ce_loss.item() * args.gradient_accumulation_steps)
                if args.fp16:
                ...
                epoch_iterator.set_description(
                    "Training (%d / %d Steps) (cls_loss=%2.5f) (shape_loss=%2.5f) (mce loss=%2.5f) (bce loss=%2.5f)" %
                    (global_step, t_total, cls_losses.val,shaping_losses.val, mask_ce_losses.val, bce_losses.val)
                )
                if args.local_rank in [-1, 0]:
                    writer.add_scalar("train/loss", scalar_value=losses.val, global_step=global_step)
                    writer.add_scalar("train/cls_loss", scalar_value=cls_losses.val, global_step=global_step)
                    writer.add_scalar("train/shaping_loss", scalar_value=shaping_losses.val, global_step=global_step)
                    writer.add_scalar("train/mce_loss", scalar_value=mask_ce_losses.val, global_step=global_step)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["cifar10", "cifar100", "soybean200","soybean2000","soybean_gene","cotton", "CUB", "dog","car","air", "AFD", "WRD","soybean_aging_R1","soybean_aging_R3","soybean_aging_R4","soybean_aging_R5","soybean_aging_R6"], default="cifar10",
    ...
    parser.add_argument('--shape_loss_weight', type=float, default=0.5,
                        help="Label smoothing value\n")
