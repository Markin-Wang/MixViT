...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            in_channels = config.resnet.num_features
            
        if config.num_parts!=0:
            n_patches = config.num_parts
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for i, layer_block in enumerate(self.layer):
            if self.mask and i == len(self.layer) - 1:
        ...
        if self.mask:
            mask_encoded, mask_idxs = self.mask_transformer(pre_states, weights, contribution)
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, mask_encoded, attn_weights, mask_idxs = self.encoder(embedding_output)
        return encoded, mask_encoded, attn_weights, mask_idxs
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, vis=False):
        ...
        self.mask_head = Linear(config.hidden_size, self.n_patches)
        self.mask_cls_head = Linear(config.hidden_size, num_classes)
        self.feature_fusion = config.feature_fusion
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            mask_ce_loss = None
            if self.mask:
                mask_labels = labels.new_zeros((x.size(0), self.n_patches)).float()
            ...
            return ce_loss, mask_ce_loss, bce_loss, logits
        else:
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                posemb = np.concatenate([posemb_tok, posemb_grid[0,], axis=1)
                
                            # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
                '''
                gs_old = int(math.sqrt(len(posemb_grid)))
                print('Resized position embedding from size:{} to size: {} with height:{} width: {}'.format(posemb.shape, posemb_new.shape, self.height, 1))
                posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
                posemb_grid = F.interpolate(posemb_grid, size=(self.height, 1), mode='bilinear')
                posemb_grid = posemb_grid.permute(0, 2, 3, 1).reshape(1, self.height, -1)
            ...
            for bname, block in self.transformer.encoder.named_children():
                if bname.startswith('ff') == False:
                    for uname, unit in block.named_children():
                        if bname.startswith('mask'): continue
...
def generate_masks(weights, contribution, topk = 0.01):

    B = weights.size(0)
    n_patches = weights.shape[-1]
    scores = weights[:, :, 0, :].mean(1)
    _ , max_inx = torch.topk(scores, int(topk * n_patches), dim=1)
